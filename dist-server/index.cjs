...
async function generateFashionKnowledge(question) {
  if (!ai) {
...
router4.post("/ai/fashion-assistant", async (req, res) => {
  const { prompt } = req.body;
  try {
    ...
    if (isSupabaseConfigured && supabaseAdmin) {
      try {
        ...
        await withTimeout(
          ...
          2500
        ).catch((uErr) => console.warn("Supabase public.users upsert note:", uErr?.message || uErr));
      } catch (e) {
...
var EmailService = class {
    ...
    if (isGmail) {
      return import_nodemailer.default.createTransport({
        ...
        auth: {
          ...
          // Strip spaces from Gmail App Password
        },
  ...
  initTransporter() {
    ...
    if (smtpUser && smtpPass) {
      ...
      console.warn('[EmailService] \u{1F4A1} To receive emails in inbox (e.g. Gmail), set SMTP_USER="your-email@gmail.com" and SMTP_PASS="your-16-digit-app-password" in .env');
    }
  }
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 12px; background-color: #ffffff;">
        ...
        <div style="border-top: 1px solid #eeeeee; padding-top: 15px; text-align: center; font-size: 11px; color: #888888;">
          <p>&copy; ${(/* @__PURE__ */ new Date()).getFullYear()} ModelVerse India. All rights reserved.</p>
        </div>
      </div>
    `;
...
// server/controllers/auth.controller.ts
init_supabase();
var resetOtpStore = /* @__PURE__ */ new Map();
...
var AuthController = class {
  ...
  static async forgotPassword(req, res) {
    try {
        ...
        if (isSupabaseConfigured && supabaseAdmin) {
          try {
            ...
            try {
              ...
              authUser = authUsers?.users?.find((u) => u.email?.toLowerCase() === targetEmail);
            } catch (e) {
            }
            if (!authUser) {
              try {
      ...
      console.log(`
========================================================================`);
      console.log(`[PASSWORD RESET OTP DISPATCH] \u{1F511} 6-Digit OTP for ${cleanTarget}: ${otpCode}`);
      console.log(`========================================================================
`);
      return res.status(200).json({
        success: true,
        otpCode,
        // Included for instant on-screen fail-safe verification & WhatsApp SMS helper
  ...
  static async resetPassword(req, res) {
    try {
      ...
      if (isSupabaseConfigured && supabaseAdmin && cleanTarget.includes("@")) {
        try {
          const { data: authUsers } = await supabaseAdmin.auth.admin.listUsers();
          const authUser = authUsers?.users?.find((u) => u.email?.toLowerCase() === cleanTarget);
          if (authUser) {
...
// server/repositories/blog.repository.ts
var import_fs19 = __toESM(require("fs"), 1);
var import_path19 = __toESM(require("path"), 1);
init_supabase();
var LOCAL_BLOGS_FILE = import_path19.default.join(process.cwd(), "local_blogs.json");
...
function generateSlug(text) {
  return text.toLowerCase().trim().replace(/[^\w\s-]/g, "").replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "") || `blog-${Date.now()}`;
}
...
function getLocalBlogs() {
  try {
    if (import_fs19.default.existsSync(LOCAL_BLOGS_FILE)) {
      const data = JSON.parse(import_fs19.default.readFileSync(LOCAL_BLOGS_FILE, "utf8"));
...
function saveLocalBlogs(blogs) {
  try {
    import_fs19.default.writeFileSync(LOCAL_BLOGS_FILE, JSON.stringify(blogs, null, 2), "utf8");
  } catch (e) {
...
function mapSupabaseRowToBlog(row) {
  ...
  return {
    ...
    publishedDate: row.published_at ? new Date(row.published_at).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" }) : row.published_date || row.publishedDate || "Jul 24, 2026",
...
var BlogRepository = class {
  async findAll(filters) {
    ...
    if (isSupabaseConfigured && supabaseAdmin) {
      try {
        ...
        const { data, error } = await withTimeout(query, 3e3);
    ...
    if (filters?.search) {
      ...
      results = results.filter(
        (b) => b.title.toLowerCase().includes(q) || b.summary.toLowerCase().includes(q) || b.content.toLowerCase().includes(q) || b.category.toLowerCase().includes(q) || b.author.toLowerCase().includes(q)
      );
    }
    return results;
  }
    ...
    if (isSupabaseConfigured && supabaseAdmin) {
      try {
        const row = {
          ...
          published_at: normalizedBlog.publishedAt || (/* @__PURE__ */ new Date()).toISOString(),
  ...
  async updateStatus(id, status, isFeatured) {
    ...
    blog.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  ...
  async getCategories() {
    ...
    return DEFAULT_CATEGORIES2;
  }
};
var blogRepository = new BlogRepository();

// server/controllers/blog.controller.ts
var BlogController = class {
  static async getBlogs(req, res) {
    try {
      const { status, category, search, role, userEmail } = req.query;
...
router26.get("/blogs", BlogController.getBlogs);
router26.get("/blogs/categories", BlogController.getCategories);
router26.get("/blog-categories", BlogController.getCategories);
router26.get("/blogs/:id", BlogController.getBlogById);
router26.post("/blogs", BlogController.saveBlog);
router26.put("/blogs/:id", BlogController.saveBlog);
router26.patch("/blogs/:id/status", BlogController.updateStatus);
router26.patch("/blogs/:id", BlogController.saveBlog);
